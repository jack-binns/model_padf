...
class ModelPADF:
    ...
    def __init__(self):
        ...
        self.project = ""
        self.tag = ""
        self.xyz_name = ""  # the xyz file contains the cartesian coords of the crystal structure expanded
        # to include r_probe
        self.subject_atom_name = ""  # the cif containing the asymmetric unit
        ...
        self.extended_atoms = []
        self.loop_similarity_array = [[1, 0.0]]
    ...
    def report_cossim(self):
        if int(self.loop_similarity_array[-1][0]) % 100 == 0:
            print("---------------------------------")
            print("Loop num        cosine similarity")
            print("---------------------------------")
            print(f'{self.loop_similarity_array[-1][0]}         {self.loop_similarity_array[-1][1]}')
            print("---------------------------------")
    ...
    def convergence_check(self, loop_number):
        if loop_number == 1:
            ...
            # print(self.loop_similarity_array)
            return 0.0
        else:
            ...
            # print(self.loop_similarity_array)
            if self.verbosity == 0:
                os.remove(f'{self.root}{self.project}{self.tag}_rolling_Theta_{loop_number-1}.npy')
            return loop_cos
        ...
        for loop_id in np.arange(1, int(self.loops) + 2, 1):
            ...
            if self.convergence_check_flag:
                if not loop_id == 1:
                    ...
                    # print(f'{loop_convergence} - {self.loop_similarity_array[loop_id - 2][1]}')
                    print(f'similarity change :: {loop_convergence - self.loop_similarity_array[loop_id - 2][1]}  target :: {self.convergence_target}')
