...
class ModelPADF:
    ...
    def write_calculation_summary(self):
        ...
        :return:
        """
        with open(self.root + self.project + 'calculation_time_log.txt', 'w') as f:
    ...
    def bin_cor_vec_to_theta(self, cor_vec, array):
        ...
        if self.dimension == 2:
            ...
            array[index_vec[0], index_vec[1]] = array[index_vec[0], index_vec[1]] + 1.0
        elif self.dimension == 3:
            r1_index = (np.abs(r_yard_stick - cor_vec[0])).argmin()
            r2_index = (np.abs(r_yard_stick - cor_vec[1])).argmin()
            th_index = (np.abs(th_yard_stick - cor_vec[-1])).argmin()
            array[r1_index, r2_index, th_index] = array[r1_index, r2_index, th_index] + 1.0
    ...
    def pair_dist_calculation(self):
        ...
        np.savetxt(self.root + self.project + self.project[:-1] + '_atomic_pairs.txt', self.n2_contacts)
        print(f'<pair_dist_calculation> ... interatomic distances calculated')
        pdf_r_range = np.arange(start=0, stop=self.r_probe, step=(self.r_dist_bin / 10))
        pdf_r_bin = self.r_dist_bin / 10
        n_atoms = len(self.extended_atoms)
        n_atom_density = n_atoms / ((4 / 3) * np.pi * self.r_probe ** 3)
        ...
        np.savetxt(self.root + self.project + self.project[:-1] + '_PDF.txt', pdf_arr)
        np.savetxt(self.root + self.project + self.project[:-1] + '_APDF.txt', np.column_stack((adfr_r, adfr_int)))
        print(f"{self.root + self.project + self.project[:-1] + '_PDF.txt'}")
        print(f"{self.root + self.project + self.project[:-1] + '_APDF.txt'}")
